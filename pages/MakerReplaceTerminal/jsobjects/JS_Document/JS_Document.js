export default {
    ...
    getRequestId: () => {
        return JS_REPLACE_TERMINAL.getRequestId();
    },
    ...
    try {
        ...
        await QRY_DELETE_DOCUMENT.run({
            documentId: row.DOCUMENT_ID,
            requestId: row.REQUEST_ID
        });

        await QRY_Documents.run();
				closeModal(mdDeleteDocument.name);
        showAlert(
...
openDeleteDocument: async () => {
    ...
    await storeValue(
        "MMS_DOCUMENT_DELETE_ID",
        row.DOCUMENT_ID
    );

    await storeValue(
        "MMS_DOCUMENT_DELETE_REQUEST_ID",
        row.REQUEST_ID
    );

    await storeValue(
        "MMS_DOCUMENT_DELETE_FILE_NAME",
        row.FILE_NAME
    );

    showModal(
        mdDeleteDocument.name
    );
},
};